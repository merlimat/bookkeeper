...
class BookieWatcherImpl implements BookieWatcher {
    ...
    private static final Function<Throwable, BKException> EXCEPTION_FUNC = cause -> {
        if (cause instanceof BKException) {
            log.error().attr("cause", cause).log("Failed to get bookie list : ");
            return (BKException) cause;
        } else if (cause instanceof InterruptedException) {
            log.error().attr("cause", cause).log("Interrupted reading bookie list : ");
        ...
        this.quarantinedBookies = CacheBuilder.newBuilder()
                ...
                .removalListener(new RemovalListener<BookieId, Boolean>() {
                    ...
                    public void onRemoval(RemovalNotification<BookieId, Boolean> bookie) {
                        log.info().attr("getKey", bookie.getKey()).log("Bookie is no longer quarantined");
                    }
            ...
            if (isEnsembleAdheringToPlacementPolicy == PlacementPolicyAdherence.FAIL) {
                ...
                if (ensembleSize > 1) {
                    log.info()
                            ...
                            .log("New ensemble: is not adhering to Placement Policy. quarantinedBookies:");
                }
            }
        ...
        try {
            ...
            if (isEnsembleAdheringToPlacementPolicy == PlacementPolicyAdherence.FAIL) {
                ...
                log.warn()
                        ...
                        .attr("quarantinedBookiesSet", quarantinedBookiesSet)
                        .log("replaceBookie for bookie: in ensemble: is not adhering to placement policy and"
 ...
 + " chose . excludedBookies ");
    ...
    public void quarantineBookie(BookieId bookie) {
        if (quarantinedBookies.getIfPresent(bookie) == null) {
            ...
            log.warn().attr("bookieAddr", bookie).log("Bookie has been quarantined because of read/write errors.");
        }
    }
