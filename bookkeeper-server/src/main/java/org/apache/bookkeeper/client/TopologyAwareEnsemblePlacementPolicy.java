            ...
            private boolean checkSumOfSubsetWithinLimit(final Set<String> includedRacksOrRegions,
                ...
                if (remainingRacksOrRegions.isEmpty() || (subsetSize <= 0)) {
                    if (maxAllowedSum < 0) {
                        ...
                        .log("CHECK FAILED: RacksOrRegions Included Remaining , subsetSize , "
 + "maxAllowedSum ");
                ...
                for (String rackOrRegion: remainingRacksOrRegions) {
                    ...
                    if (currentAllocation > maxAllowedSum) {
                        ...
                        .attr("subsetSize", subsetSize)
                        .attr("maxAllowedSum", maxAllowedSum)
                        .log("CHECK FAILED: RacksOrRegions Included Candidate , subsetSize , "
 + "maxAllowedSum ");
    ...
    static class DNSResolverDecorator implements DNSToSwitchMapping {
        ...
        public List<String> resolve(List<String> names) {
            ...
            if (rNames != null && rNames.size() == names.size()) {
                for (int i = 0; i < rNames.size(); ++i) {
                    if (rNames.get(i) == null) {
                        log.warn()
                                .attr("get", names.get(i))
                                .attr("defaultRack", defaultRack)
                                .log("Failed to resolve network location for , using default rack for it : .");
            ...
            log.warn()
                    .attr("names", names)
                    .attr("defaultRack", defaultRack)
                    .log("Failed to resolve network location for , using default rack for them : .");
    ...
    public Set<BookieId> onClusterChanged(Set<BookieId> writableBookies,
        ...
        try {
            ...
            .log("Cluster changed : left bookies are , joined bookies are , while dead bookies are .");
        ...
        for (BookieId addr : leftBookies) {
            try {
                ...
                if (null != node) {
                    ...
                    log.debug().attr("bookieAddr", addr).log("Cluster changed : bookie left from cluster.");
    ...
    protected String resolveNetworkLocation(BookieId addr) {
        try {
            ...
            log.error()
                    .attr("bookieAddr", addr)
                    .attr("defaultRack", defaultRack)
                    .attr("getMessage", err.getMessage())
                    .log("Cannot resolve bookieId to a network address, resolving as .");
            return defaultRack;
        }
    }
