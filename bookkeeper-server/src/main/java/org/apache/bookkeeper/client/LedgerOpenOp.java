...
class LedgerOpenOp {
    ...
    private void openWithMetadata(Versioned<LedgerMetadata> versionedMetadata) {
        ...
        try {
            ...
            log.error().exception(e).log("Security exception while opening ledger: " + ledgerId);
            openComplete(BKException.Code.DigestNotInitializedException, null);
            return;
        } catch (NumberFormatException e) {
            log.error().exception(e).log("Incorrectly entered parameter throttle: " + bk.getConf().getThrottleValue());
