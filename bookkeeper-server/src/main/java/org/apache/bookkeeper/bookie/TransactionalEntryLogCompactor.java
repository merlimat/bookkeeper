    ...
    public boolean compact(EntryLogMetadata metadata) {
        if (metadata != null) {
            log.info()
                    ...
                    .log("Compacting entry log with usage .");
        ...
        boolean run() {
            try {
                ...
                log.error()
                        ...
                        .log("Encounter exception in compaction phase . Abort current compaction.");
                abort();
            }
            return false;
        }
                    ...
                    synchronized (TransactionalEntryLogCompactor.this) {
                        ...
                        if (lid != ledgerId || entryId < -1) {
                            log.warn()
                                    ...
                                    .attr("offset", offset)
                                    .log("Scanning expected ledgerId , but found invalid entry "
                        ...
                        offsets.add(new EntryLocation(ledgerId, entryId, newOffset));
                                    log.debug()
                                            ...
                                            .log("Compact add entry : lid = , eid = , offset =");
                    }
                }
            });
        }
