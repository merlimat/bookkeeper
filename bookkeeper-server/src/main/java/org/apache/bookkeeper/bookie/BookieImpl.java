...
public class BookieImpl implements Bookie {
    ...
    static class NopWriteCallback implements WriteCallback {
        ...
        public void writeComplete(int rc, long ledgerId, long entryId,
            ...
            log.debug()
                    ...
                    .attr("addr", addr)
                    .attr("rc", rc)
                    .log("Finished writing entry @ ledger for");
        }
    }
    ...
    private void checkIfDirsOnSameDiskPartition(List<File> dirs) throws DiskPartitionDuplicationException {
        ...
        for (File dir : dirs) {
            ...
            try {
                ...
                log.error().attr("dir", dir).log("Got IOException while trying to FileStore of");
    ...
    void readJournal() throws IOException, BookieException {
            ...
            public void process(int journalVersion, long offset, ByteBuffer recBuff) throws IOException {
                ...
                try {
                    log.debug()
                            .attr("ledgerId", ledgerId)
                            .attr("entryId", entryId)
                            .log("Replay journal - ledger id : , entry id : .");
                    if (entryId == METAENTRY_ID_LEDGER_KEY) {
                        ...
                        log.warn()
                                .attr("entryId", entryId)
                                .attr("ledgerId", ledgerId)
                                .log("Read unrecognizable entryId: for ledger: while replaying Journal. Skipping it");
                    } else {
                        byte[] key = masterKeyCache.get(ledgerId);
                        if (key == null) {
                            key = ledgerStorage.readMasterKey(ledgerId);
        ...
        log.info().attr("elapsedTs", elapsedTs).log("Finished replaying journal in ms.");
    }
    ...
    private void replay(Journal journal, JournalScanner scanner) throws IOException {
        ...
        for (Long id : logs) {
            ...
            log.info()
                    ...
                    .log("Replaying journal from position");
    ...
    public synchronized void start() {
        ...
        log.debug()
                .attr("stream", journalDirectories.stream()
                        .map(File::getName).collect(Collectors.joining(", ")))
                .log("I'm starting a bookie with journal directories");
        ...
        try {
            ...
            log.error().exception(e).log("Couldn't register bookie with zookeeper, shutting down : ");
    ...
    void triggerBookieShutdown(final int exitCode) {
        ...
        log.info()
                ...
                .log("Triggering shutdown of Bookie- with exitCode");
        ...
        try {
            if (isRunning()) {
                ...
                log.info()
                        .attr("bookiePort", conf.getBookiePort())
                        .attr("exitCode", exitCode)
                        .log("Shutting down Bookie- with exitCode");
    ...
    public static boolean format(ServerConfiguration conf,
        ...
        for (File dir : ledgerDirs) {
            if (!cleanDir(dir)) {
                log.error().attr("dir", dir).log("Formatting ledger directory failed");
                return false;
            }
        }
        ...
        if (null != indexDirs) {
            for (File dir : indexDirs) {
                if (!cleanDir(dir)) {
                    log.error().attr("dir", dir).log("Formatting index directory failed");
                    return false;
                }
            }
        }
    ...
    private static boolean cleanDir(File dir) {
        if (dir.exists()) {
            ...
            log.error().attr("dir", dir).log("Not able to create the directory");
            return false;
        }
        return true;
    }
