...
public class FileSystemUpgrade {
    ...
    private static void upgrade(ServerConfiguration conf,
        ...
        try {
            ...
            for (File d : getAllDirectories(conf)) {
                log.info().attr("d", d).log("Upgrading");
                ...
                try {
                    ...
                    log.error().attr("d", d).log("Error upgrading");
    ...
    public static void finalizeUpgrade(ServerConfiguration conf)
        ...
        for (File d : getAllDirectories(conf)) {
            log.info().attr("d", d).log("Finalizing");
            try {
                int version = detectPreviousVersion(d);
                if (version < 3) {
                    if (version == 2) {
                    ...
                    if (files != null) {
                        for (File f : files) {
                            if (f.isDirectory()) {
                                ...
                                if (!f.delete()) {
                                    log.warn().attr("f", f).log("Could not delete");
                                }
                            }
                        }
                    }
                ...
                log.error().attr("d", d).log("Error finalizing");
                throw new BookieException.UpgradeException(ioe);
            }
        }
        // noop at the moment
    ...
    private static void rollback(ServerConfiguration conf,
        ...
        for (File d : getAllDirectories(conf)) {
            log.info().attr("d", d).log("Rolling back");
            try {
                ...
                log.error().attr("d", d).log("Error rolling back");
                throw new BookieException.UpgradeException(ioe);
            }
        }
        try {
