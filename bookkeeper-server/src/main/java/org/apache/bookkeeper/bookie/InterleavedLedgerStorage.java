        ...
        for (EntryLocation l : locations) {
            try {
                ...
                // Ledger was already deleted, we can skip it in the compaction
                            log.debug()
                                    .attr("ledger", l.ledger)
                                    .attr("entry", l.entry)
                                    .log("Compaction failed for deleted ledger ledger: entry");
            }
        }
    }
            ...
            try (LedgerCache.PageEntriesIterable pages = ledgerCache.listEntries(ledger)) {
                ...
                if (activeLedgers.containsKey(ledger)) {
                    log.error()
                            .exception(e)
                            .attr("ledger", ledger)
                            .log("Cannot find ledger , should exist, exception is");
                    errors.add(new DetectedInconsistency(ledger, -1, e));
                } else {
                    log.debug().attr("ledger", ledger).log("ledger deleted since snapshot taken");
                }
            } catch (Exception e) {
