...
public final class CustomZooKeeperHostProvider implements HostProvider {
    ...
    public CustomZooKeeperHostProvider(Collection<InetSocketAddress> serverAddresses) {
        init(serverAddresses, System.currentTimeMillis() ^ this.hashCode(), new CustomResolver());
    }
     ...
     */
    public CustomZooKeeperHostProvider(Collection<InetSocketAddress> serverAddresses, Resolver resolver) {
    ...
    public synchronized boolean updateServerList(
        ...
        boolean myServerInNewConfig = isMyServerInNewConfig(resolver, shuffledList, myServer);
    ...
    static boolean isMyServerInNewConfig(StaticHostProvider.Resolver resolver, List<InetSocketAddress> connectAddresses,
        ...
        for (InetSocketAddress unresolvedConnectAddr : connectAddresses) {
            ...
            if (unresolvedConnectAddr.getPort() == myServer.getPort()) {
                ...
                if (!myServer.isUnresolved() && connectHostName.equals(myServer.getAddress().getHostAddress())) {
                    return true;
                }
                InetAddress[] resolvedConnectAddresses = null;
                try {
                    // perform lookup for all ip addresses of a single connect address
                    resolvedConnectAddresses = resolver.getAllByName(connectHostName);
                } catch (UnknownHostException e) {
                    // ignore
                }
                if (!myServer.isUnresolved() && resolvedConnectAddresses != null) {
                    // check if any of the resolved IP addresses for a single connect address match
                    for (InetAddress connectAddr : resolvedConnectAddresses) {
                        if (Arrays.equals(connectAddr.getAddress(), myServer.getAddress().getAddress())) {
                            return true;
                        }
                    }
                }
                try {
                    // perform reverse lookup and then a lookup with the returned myServer host name
                    // to get all ip addresses in the case when the DNS names has multiple ip addresses
                    InetAddress[] resolvedMyServerAddresses = resolver.getAllByName(myServer.getHostName());
                    // check if any ip address matches any ip of a resolved connect address
                    ...
                    // check reverse lookup for each ip address if there's match by the connect host name
                    for (InetAddress myServerAddr : resolvedMyServerAddresses) {
    ...
    private static class CustomResolver implements StaticHostProvider.Resolver {
        ...
        public InetAddress[] getAllByName(String name) throws UnknownHostException {
            ...
            InetAddress numericIpAddress = NetUtil.createInetAddressFromIpAddressString(name);
